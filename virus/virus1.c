...
// Function to process a file and replace suspicious strings
void process_file(const char *path) {
    ...
    while (fgets(buffer, sizeof(buffer), file) != NULL) {
        for (int i = 0; i < num_strings; ++i) {
            ...
            if (new_str != buffer) {
                fseek(file, -strlen(buffer), SEEK_CUR); // Move back to overwrite the line
                fputs(new_str, file); // Write the modified line
                replaced = 1;
                // Log the replacement
                time_t now;
...
int main(int argc, char *argv[]) {
    ...
    // Main loop to process directory periodically
    while (1) {
